...
class Solver {
  ...
  solve() {
    const result: any = this.solveNarrow(this.solveWide());
    return {
  ...
  spsa(A: number, a: any[], c: number, values: string | any[], iters: number) {
    ...
    for (let k = 0; k < iters; k++) {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        (values as any)[i] = fix(values[i] - ak * g, i);
      }

      const loss = this.loss((values as any));
      if (loss < bestLoss) {
        best = values.slice(0);
        bestLoss = loss;
      }
...
export function hexToFilter(hex: any) {
